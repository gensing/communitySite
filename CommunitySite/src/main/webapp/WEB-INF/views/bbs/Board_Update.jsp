...
<script>
	...
	function addFile() {
		...
		fileInput.bind("change", function() {
			...
			$(wrap).append($("<p>업로드 파일:" + this.files[0].name + "<input type='button' value='취소' onclick='deleteFile(this)'></P>"));
	...
	function oldFileDel(wrap) {
		//$(wrap).parent().parent().children("input").attr("name", "delFile");
		//$(wrap).parent().parent().attr("style", "display: none");

		$(wrap).next("input").attr("name", "delFile");
		$(wrap).parent().attr("style", "display: none");
	}
</script>
	...
	<div class="container" style="min-height: 800px;">
		...
		<div class="col-sm-10 col-sm-offset-1">
			<form action="${contextPath }/updateBoard" method="post" enctype="multipart/form-data">
				...
				<input type="text" name="title" id="search-field" class="form-control" placeholder="제목을 입력하세요" value="${board.title }" />
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						<div id="oldFileList">
							<c:forEach items="${file }" var="oldFile">
								<p>${oldFile.fileRealName }<input type="button" value="삭제" onclick="oldFileDel(this)" /> <input type="text" value="${oldFile.fileRealName }" style="display: none;">
								</p>
							</c:forEach>
